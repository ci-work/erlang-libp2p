...
handle_info({handle_identify, {ReplyRef, StreamPid, Path}, {ok, Identify}}, State=#state{}) ->
    ...
    case lookup_worker(Target, #worker.target, State) of
        ...
        false ->
            lager:info("received identity for non existing target ~p.  Stream: ~p",[Target, StreamPid]),
            case count_workers(inbound, State) > State#state.max_inbound_connections of
                true ->
                    lager:info("Too many inbound workers: ~p",
        ...
        #worker{pid=Worker} ->
            lager:info("received identity for existing target ~p.  Stream: ~p",[Target, StreamPid]),
...
-spec get_fixed_destination_peer(State :: term()) -> string().
get_fixed_destination_peer(State=#state{tid=TID}) ->
    {CurrentAddrs, _} = lists:unzip(connections(all, State)),
    LocalAddr = libp2p_swarm:p2p_address(TID),
    ExcludedAddrs = CurrentAddrs ++ [LocalAddr],
    ...
    lists:nth(rand:uniform(length(BaseAddrs)), BaseAddrs).
